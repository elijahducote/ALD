...
import { print, collectPaths } from "./lib/utility.js";
...
async function processHTML(nav, pageName = null) {
  ...
  const pagesToProcess = pageName ? [pageName] : paths;

  // Build name→page map
  const pageMap = {};
  {
    let fI = nav.siteIndex.length, fCur;
    for (; fI; --fI) {
      fCur = nav.siteIndex.length - fI;
      pageMap[nav.siteIndex[fCur].name] = nav.siteIndex[fCur];
    }
  }
  ...
  try {
    ...
    {html: output} = await createFavicon({
      sourceFile: './nav/icons/ald_fullcolour.svg',
  ...
  for (const nomer of pagesToProcess) {
    ...
    webDQment = html({lang:"en-US"},
      await MetaData(tags, page, icons, nav.name),
      Body(tags, {
        //header: TopNav(tags),
